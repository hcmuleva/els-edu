...
function shouldUpdateUser(user, replaceAll, addOnly) {
  const currentRoles = getUserRoles(user.assigned_roles);
...
async function main() {
  ...
  try {
    ...
    const result = await client.query(`
      SELECT id, username, email, user_role, assigned_roles, document_id
    ...
    const usersToUpdate = users.filter((user) =>
      shouldUpdateUser(user, replaceAll, addOnly)
    );
    ...
    if (!skipConfirm) {
      console.log(
        "⚠️  Ready to update. Press Ctrl+C to cancel, or wait 3 seconds to continue...\n"
      );
    ...
    for (let i = 0; i < usersToUpdate.length; i++) {
      ...
      try {
        ...
        if (successCount % 10 === 0 && i > 0) {
          console.log(
            `📊 Progress: ${i + 1}/${usersToUpdate.length} users processed...`
          );
        }
      } catch (error) {
        errorCount++;
