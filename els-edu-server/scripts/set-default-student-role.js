 ...
 * 2. Set user_roles (JSON array) to [{ role: "STUDENT" }] for users who don't have any roles (default)
...
const STRAPI_URL = process.env.STRAPI_URL || process.env.BACKEND_URL || 'http://localhost:1337';
// Get API token from environment or command line
const args = process.argv.slice(2);
const tokenIndex = args.indexOf('--token');
const API_TOKEN = tokenIndex !== -1 && args[tokenIndex + 1] 
    ? args[tokenIndex + 1] 
    ...
    try {
        ...
        while (hasMore) {
            ...
            if (Array.isArray(response.data)) {
                ...
                console.log(`⚠️  Response.data.data is not an array, trying to parse...`);
            } else {
                console.log(`⚠️  Unexpected response structure:`, JSON.stringify(response.data, null, 2).substring(0, 500));
            }

            allUsers = allUsers.concat(users);

            const pagination = response.data.meta?.pagination || response.data?.meta?.pagination;
            console.log(`Pagination info:`, pagination);
            
            if (pagination) {
                hasMore = page < pagination.pageCount;
                console.log(`Page ${page} of ${pagination.pageCount}, hasMore: ${hasMore}`);
            } else {
                // If no pagination info, assume no more pages if we got fewer users than pageSize
                hasMore = users.length === pageSize;
                console.log(`No pagination info, hasMore: ${hasMore} (got ${users.length} users, pageSize: ${pageSize})`);
            }
            
            page++;

            console.log(`✅ Fetched page ${page - 1}: ${users.length} users (Total so far: ${allUsers.length})`);
        ...
        if (allUsers.length > 0) {
            console.log(`Sample user structure:`, JSON.stringify(allUsers[0], null, 2).substring(0, 300));
        }
...
function getUserRoles(user) {
    ...
    let rolesArray = user.user_roles;
...
function shouldUpdateUser(user) {
    ...
    // Check if user_role needs to be updated to match user_roles
    ...
    // Also update if user_role doesn't match the roles in user_roles or is null/undefined
...
function getActiveRole(user) {
    // user_role should match the roles in user_roles
    // Priority: SUPERADMIN > ADMIN > TEACHER > PARENT > MARKETING > STUDENT
    // Map user_roles to user_role based on what roles the user has
    const currentRoles = getUserRoles(user);
    
    // Priority: SUPERADMIN > ADMIN > TEACHER > PARENT > MARKETING > STUDENT
    // If user has SUPERADMIN in user_roles, set user_role to "SUPERADMIN"
    if (currentRoles.includes('SUPERADMIN')) {
        return "SUPERADMIN";
    }
    
    // If user has ADMIN in user_roles, set user_role to "ADMIN"
    if (currentRoles.includes('ADMIN')) {
        return "ADMIN";
    }
    
    // If user has TEACHER in user_roles, set user_role to "TEACHER"
    if (currentRoles.includes('TEACHER')) {
        return "TEACHER";
    }
    
    // If user has PARENT in user_roles, set user_role to "PARENT"
    if (currentRoles.includes('PARENT')) {
        return "PARENT";
    }
    
    // If user has MARKETING in user_roles, set user_role to "MARKETING"
    if (currentRoles.includes('MARKETING')) {
        return "MARKETING";
    }
    
    // If user has STUDENT in user_roles, set user_role to "STUDENT"
    if (currentRoles.includes('STUDENT')) {
        return "STUDENT";
    }
...
async function runMigration() {
    try {
        ...
        console.log(`   URL: ${STRAPI_URL}/api/user-update/migrations/set-default-student-roles`);
        ...
        console.error(`\n❌ Migration endpoint failed:`, error.response?.status, error.message);
...
async function main() {
    ...
    if (!API_TOKEN) {
        ...
        console.error('  1. Set API_TOKEN environment variable: API_TOKEN=your_token node scripts/set-default-student-role.js');
        console.error('  2. Use --token flag: node scripts/set-default-student-role.js --token your_token');
    ...
    try {
        ...
        if (result.success) {
            ...
            console.log(`⏭️  Skipped (no changes needed): ${result.result.skipped} users`);
        ...
        if (error.response) {
            ...
            if (error.response.status === 401) {
                ...
                console.error('   Try restarting your Strapi server to register the new route');
            }
        }
        process.exit(1);
    }
