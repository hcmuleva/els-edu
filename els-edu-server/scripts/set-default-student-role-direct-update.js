    ...
    try {
        ...
        try {
            ...
            if (err.response?.status === 405) {
                throw new Error('ROUTE_NOT_REGISTERED: Server needs restart to register custom routes');
            }
        }
        ...
        try {
            ...
            if (updated && (updated.user_role === activeRole || JSON.stringify(updated.user_roles) === JSON.stringify(newRoles))) {
                return {
                    success: true,
                    user_roles: updated.user_roles,
                    user_role: updated.user_role,
        ...
        return { 
            success: false, 
            error: 'Update endpoints not working. Server may need restart or fields are read-only via REST API.' 
        };
    } catch (error) {
...
async function main() {
    ...
    if (!API_TOKEN) {
        ...
        console.error('  node scripts/set-default-student-role-direct-update.js --token YOUR_TOKEN --yes');
        console.error('');
        process.exit(1);
    }
    ...
    try {
        ...
        usersToUpdate.slice(0, 5).forEach(user => {
            ...
            console.log(`    Current: [${currentRoles.join(', ') || 'none'}] -> New: [${newRoles.map(r => r.role).join(', ')}]`);
        });
        ...
        for (let i = 0; i < usersToUpdate.length; i++) {
            ...
            try {
                ...
                if (result.success) {
                    ...
                    if (i < 5) {
                        console.error(`❌ Failed user ${user.id} (${user.username || user.email}): ${result.error}`);
                    }
                }
                ...
                if (error.message.includes('ROUTE_NOT_REGISTERED')) {
                    ...
                    console.error('');
                    console.error('The update endpoints require the server to be restarted.');
                    console.error('');
                    ...
                    console.error('');
                    console.error(`Progress so far: ${successCount} updated, ${errorCount + 1} failed`);
                    process.exit(1);
                }
                errorCount++;
                if (i < 5) {
...
module.exports = { main, getAllUsers, updateUser, getUserRoles, getNewRoles, getActiveRole };
