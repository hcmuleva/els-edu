...
module.exports = createCoreController('api::quiz-result.quiz-result', ({ strapi }) => ({
    ...
    async submitResult(ctx) {
        try {
            const { quizId, answers, timeTaken, questionTimings, deviceInfo } = ctx.request.body;
            ...
            // Check attempt limit
            const existingAttempts = await strapi.documents('api::quiz-result.quiz-result').findMany({
                filters: {
            ...
            // Calculate results
            const analysis = await this.calculateQuizAnalysis(quiz, answers, questionTimings);
            ...
            results.forEach(result => {
                if (result.retakeQuestions && Array.isArray(result.retakeQuestions)) {
                    result.retakeQuestions.forEach(questionId => {
                        ...
                        const questionAnalysis = result.questionAnalysis?.find(q => q.questionId === questionId);
                        if (questionAnalysis && !questionAnalysis.isCorrect) {
                            question.failures++;
                        }

                        // Calculate priority based on failure rate and recency
                        question.priority = (question.failures / question.attempts) *
                            (1 / Math.max(1, Math.floor((Date.now() - new Date(question.lastAttempt)) / (24 * 60 * 60 * 1000))));
                    });
                }
            });
    ...
    async getLeaderboard(ctx) {
        try {
            const { quizId, subjectId, topicId, period = '30d', limit = 10 } = ctx.query;
    ...
    async calculateQuizAnalysis(quiz, answers, questionTimings) {
        ...
        questions.forEach((question, index) => {
            const userAnswer = answers.find(a => a.questionId === question.documentId);
        ...
        return {
            ...
            retakeQuestions,
            improvementSuggestions: this.generateImprovementSuggestions(weakAreas, difficultyPerformance, percentage)
        };
    },
    ...
    generateImprovementSuggestions(weakAreas, difficultyPerformance, percentage) {
        const suggestions = [];

        if (percentage < 50) {
            suggestions.push('Consider reviewing the basic concepts before retaking the quiz');
        } else if (percentage < 70) {
            suggestions.push('You\'re close to passing! Focus on your weak areas and try again');
        }

        weakAreas.forEach(area => {
            suggestions.push(`Focus more time studying ${area.topic} - you got ${area.percentage}% correct`);
        });

        Object.entries(difficultyPerformance).forEach(([difficulty, perf]) => {
            const difficultyPercentage = Math.round((perf.correct / perf.total) * 100);
            if (difficultyPercentage < 60) {
                suggestions.push(`Practice more ${difficulty} level questions - current performance: ${difficultyPercentage}%`);
            }
        });
    ...
    buildDateFilter(period, startDate, endDate) {
        ...
        const startDateCalc = new Date(now.getTime() - (days * 24 * 60 * 60 * 1000));
    ...
    calculateTrends(results) {
        ...
        results.forEach(result => {
            ...
            // Subject breakdown
            const subjectName = result.subject?.name || result.quiz?.subject?.name || 'Unknown';
            ...
            trends.subjectBreakdown[subjectName].totalPercentage += result.percentage || 0;
            ...
            trends.weeklyProgress[weekKey].totalPercentage += result.percentage || 0;
        ...
        if (results.length >= 4) {
            ...
            const firstAvg = firstHalf.reduce((sum, r) => sum + (r.percentage || 0), 0) / firstHalf.length;
            const secondAvg = secondHalf.reduce((sum, r) => sum + (r.percentage || 0), 0) / secondHalf.length;
        ...
        Object.keys(trends.subjectBreakdown).forEach(subject => {
            ...
            subjectData.averagePercentage = Math.round(subjectData.totalPercentage / subjectData.attempts);
            subjectData.passRate = Math.round((subjectData.passed / subjectData.attempts) * 100);
    ...
    calculatePerformanceBySubject(results) {
        ...
        return Object.entries(subjectPerformance).map(([subject, subjectResults]) => ({
            subject,
            averageScore: subjectResults.reduce((sum, r) => sum + (r.percentage || 0), 0) / subjectResults.length,
            attempts: subjectResults.length,
            passRate: subjectResults.filter(r => r.isPassed).length / subjectResults.length * 100
        }));
    },
    ...
    calculatePerformanceByTopic(results) {
        ...
        return Object.entries(topicPerformance).map(([topic, topicResults]) => ({
            topic,
            averageScore: topicResults.reduce((sum, r) => sum + (r.percentage || 0), 0) / topicResults.length,
            attempts: topicResults.length,
            passRate: topicResults.filter(r => r.isPassed).length / topicResults.length * 100
        }));
    },

    calculatePerformanceByDifficulty(results) {
        // Extract difficulty from question analysis
        const difficultyPerformance = { beginner: [], intermediate: [], advanced: [] };

        results.forEach(result => {
            if (result.questionAnalysis) {
                result.questionAnalysis.forEach(qa => {
                    const difficulty = qa.difficulty || 'beginner';
                    if (!difficultyPerformance[difficulty]) difficultyPerformance[difficulty] = [];
                    difficultyPerformance[difficulty].push(qa);
                });
            }
        });

        return Object.entries(difficultyPerformance).map(([difficulty, questions]) => {
    ...
    calculateMovingAverage(results, window = 3) {
        ...
        return recentResults.reduce((sum, r) => sum + (r.percentage || 0), 0) / recentResults.length;
    },
    ...
    calculateTimeImprovement(results) {
        ...
        const firstAvgTime = firstHalf.reduce((sum, r) => sum + (r.timeTaken || 0), 0) / firstHalf.length;
        const secondAvgTime = secondHalf.reduce((sum, r) => sum + (r.timeTaken || 0), 0) / secondHalf.length;
    ...
    generateRecommendations(results) {
        ...
        if (timeAnalysis.averageTime > 300) { // 5 minutes
            recommendations.push({
