...
module.exports = createCoreService(
  ...
  ({ strapi }) => ({
    ...
    async getQuizStatistics(quizId, filters = {}) {
      try {
        ...
        const averageScore =
          ...
          totalAttempts;
        const passRate =
          (results.filter((r) => r.isPassed).length / totalAttempts) * 100;
        ...
        const performanceDistribution =
          this.calculatePerformanceDistribution(results);
        ...
        return {
          ...
          recommendations: this.generateQuizRecommendations(
            results,
            questionStats
          ),
        };
    ...
    calculateRetakeRate(results) {
      ...
      const usersWithMultipleAttempts = Array.from(
        userAttempts.values()
      ...
      return totalUsers > 0
        ? Math.round((usersWithMultipleAttempts / totalUsers) * 100)
        : 0;
    },
    ...
    generateQuizRecommendations(results, questionStats) {
      ...
      if (avgTime > 1800) {
        ...
        recommendations.push({
          ...
          priority: "medium",
          message:
            "Average completion time is quite high, consider reviewing time limits",
    ...
    calculateRecentTrend(results) {
      ...
      const recentAvg =
        recent.reduce((sum, r) => sum + (r.percentage || 0), 0) / recent.length;
      const previousAvg =
        previous.reduce((sum, r) => sum + (r.percentage || 0), 0) /
        previous.length;
