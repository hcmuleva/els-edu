...
const { setDefaultStudentRoles } = require('../../../utils/migrations/set-default-student-roles');
...
module.exports = {
  ...
  async updateByDocumentId(ctx) {
    try {
      ...
      // Also allow if updating user_roles/user_role via API token (for script usage)
      ...
      const isUpdatingRoles = payload.user_roles !== undefined || payload.user_role !== undefined;
      ...
      const allowedFields = ['first_name', 'last_name', 'mobile_number', 'user_experience_level', 'user_role', 'user_roles'];
      const data = {};
  ...
  async runSetDefaultStudentRoles(ctx) {
    try {
      ...
      // Run the migration
      const result = await setDefaultStudentRoles(strapi, { replaceAll, addOnly });
      ...
      return ctx.internalServerError('Migration failed', { error: error.message });
    }
  },
