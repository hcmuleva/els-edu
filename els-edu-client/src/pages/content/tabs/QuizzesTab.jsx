...
import { useGetList, useGetIdentity, Loading, useDelete, useNotify, ReferenceField, TextField, useRedirect } from 'react-admin';
import { BookOpen, Eye, Edit2, Trash2, Search, ArrowUpDown, ArrowUp, ArrowDown, X, CheckCircle, RotateCcw } from 'lucide-react';
...
const QuizViewModal = ({ quiz, onClose }) => {
    ...
    const getDifficultyColor = (difficulty) => {
        ...
        if (difficulty === 'advanced') return 'bg-red-100 text-red-700 border-red-200';
            ...
            <div className="bg-white rounded-3xl max-w-3xl w-full max-h-[85vh] overflow-y-auto shadow-2xl animate-in zoom-in-95 duration-200 [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none] border border-border/50" onClick={(e) => e.stopPropagation()}>
                ...
                <div className="sticky top-0 bg-white/95 backdrop-blur-md border-b border-border/50 p-6 flex items-center justify-between z-10">
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            <h2 className="text-2xl font-black text-gray-900">Quiz Details</h2>
                            <p className="text-sm text-gray-500 font-medium">Reviewing quiz configuration</p>
                        </div>
                    </div>
                    <button 
                        onClick={onClose} 
                ...
                <div className="p-8 space-y-8">
                    <div className="space-y-3">
                        <div className="flex items-center gap-2 mb-2">
                            <span className="text-xs font-bold text-gray-400 uppercase tracking-widest">Quiz Title</span>
                        </div>
                        <div>
                            <p className="text-2xl font-bold text-gray-800 leading-relaxed">{quiz.title}</p>
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-6 py-2">
                        <div className="flex flex-col gap-1">
                            ...
                            <span className="font-bold text-gray-900 capitalize">{quiz.quizType}</span>
                        </div>
                        <div className="flex flex-col gap-1">
                            <span className="text-xs font-semibold text-gray-400 uppercase tracking-wider">Difficulty</span>
                            <span className={`font-bold capitalize inline-flex items-center px-2 py-0.5 rounded-lg w-fit text-sm ${getDifficultyColor(quiz.difficulty)}`}>
                                {quiz.difficulty || 'beginner'}
                            </span>
                        </div>
                        <div className="flex flex-col gap-1">
                            <span className="text-xs font-semibold text-gray-400 uppercase tracking-wider">Time Limit</span>
                            <span className="font-bold text-gray-900">{quiz.timeLimit} mins</span>
                        </div>
                        <div className="flex flex-col gap-1">
                            <span className="text-xs font-semibold text-gray-400 uppercase tracking-wider">Pass Score</span>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6 p-6 bg-gray-50 rounded-2xl border border-gray-100">
                        <div className="flex flex-col gap-1">
                            ...
                            <div className="font-bold text-gray-900 text-lg">
                                <ReferenceField record={quiz} source="subject.id" reference="subjects" link={false}>
                        ...
                        <div className="flex flex-col gap-1">
                            ...
                            <div className="font-bold text-gray-900 text-lg">
                                <ReferenceField record={quiz} source="topic.id" reference="topics" link={false}>
                                    <TextField source="name" />
                                </ReferenceField>
                            </div>
                        </div>
                        <div className="flex flex-col gap-1 md:col-span-2">
                             ...
                             <div className="font-bold text-gray-900 flex items-center gap-2">
                                ...
                                <span className="text-sm font-medium text-gray-500">questions included</span>
                             </div>
                        </div>
                    </div>
                    ...
                    {quiz.instructions && (
                        <div className="bg-blue-50/50 border border-blue-100 rounded-2xl p-6">
                            <h4 className="text-sm font-bold text-blue-900 mb-1">Instructions</h4>
                            <p className="text-blue-800/80 text-sm leading-relaxed">{quiz.instructions}</p>
                        </div>
                    )}
                </div>
            </div>
...
export const QuizzesTab = () => {
    ...
    const { data: quizzes, isLoading, refetch } = useGetList('quizzes', {
    ...
    const getFilteredContent = () => {
        ...
        if (topicFilter) {
            content = content.filter(item => {
                ...
                return tId === topicFilter || (typeof tId === 'object' && tId?.id === topicFilter);
            });
        }
        ...
        if (subjectFilter) {
            content = content.filter(item => {
                ...
                return sId === subjectFilter || (typeof sId === 'object' && sId?.id === subjectFilter);
            });
        }
    ...
    const getDifficultyColor = (difficulty) => {
        ...
        if (difficulty === 'advanced') return 'bg-red-100 text-red-700 border-red-300';
        return 'bg-orange-100 text-orange-700 border-orange-300';
    };

    return (
        <div className="space-y-6">
            {viewingQuiz && <QuizViewModal quiz={viewingQuiz} onClose={() => setViewingQuiz(null)} />}
            ...
            {isLoading ? (
                ...
                <div className="flex flex-col items-center justify-center h-64 text-center text-muted-foreground p-6 bg-white rounded-b-3xl">
                    ...
                    <p className="text-sm opacity-70 mt-1">Try adjusting filters or create a new quiz</p>
                </div>
            ) : (
                    ...
                    <table className="w-full">
                        <thead className="bg-gray-50 border-b border-border/50 sticky top-0 z-10">
                            <tr>
                                ...
                                <th className="px-6 py-4 text-left text-xs font-bold text-gray-600 uppercase tracking-wider">Quiz Title</th>
                                <th className="px-6 py-4 text-center text-xs font-bold text-gray-600 uppercase tracking-wider">Type</th>
                                <th className="px-6 py-4 text-center text-xs font-bold text-gray-600 uppercase tracking-wider">Difficulty</th>
                                <th className="px-6 py-4 text-center text-xs font-bold text-gray-600 uppercase tracking-wider">Time (min)</th>
                                <th className="px-6 py-4 text-center text-xs font-bold text-gray-600 uppercase tracking-wider">Pass Score</th>
                                <th className="px-6 py-4 text-center text-xs font-bold text-gray-600 uppercase tracking-wider">Active</th>
                                ...
                                return (
                                    <tr key={itemId} className="hover:bg-gray-50/50 transition-colors">
                                        ...
                                        <td className="px-6 py-4 align-middle text-center">
                                            <span className={`inline-block w-3 h-3 rounded-full ${item.isActive ? 'bg-green-500' : 'bg-red-500'}`} />
                                        </td>
                                        <td className="px-6 py-4 align-middle">
                                            <span className="text-sm text-gray-500">
                                                <ReferenceField record={item} source="subject.id" reference="subjects" link={false}>
                                                    <TextField source="name" />
                                                </ReferenceField>
                                            </span>
                                        </td>
                                        <td className="px-6 py-4 align-middle">
                                            <span className="text-sm text-gray-500">
                                                <ReferenceField record={item} source="topic.id" reference="topics" link={false}>
                                                    <TextField source="name" />
                                                </ReferenceField>
                                            </span>
                                        </td>
