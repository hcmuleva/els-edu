...
const TeachersTab = () => {
  ...
  const { data, isLoading, refetch } = useQuery(
    ['teachers', page + 1, rowsPerPage, search],
    () => managementApi.getUsersByRole('teacher', page + 1, rowsPerPage, search),
    {
        ...
        <Table>
          ...
          <TableBody>
            {teachers.map((teacher) => {
              const experienceLevel = getExperienceLevel(teacher.user_experience_level);
                ...
                <TableRow key={teacher.id} hover>
                  <TableCell>
                    <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
                      <Avatar
                        src={teacher.profile_photo?.url}
                        sx={{ width: 40, height: 40, bgcolor: 'warning.light' }}
                      >
                        {teacher.first_name?.[0] || teacher.username?.[0] || 'U'}
                      </Avatar>
                      <Box>
                  ...
                  <TableCell>
                    <Chip
                      label={teacher.user_status || 'PENDING'}
                      size="small"
                      color={getStatusColor(teacher.user_status)}
                      variant="filled"
                    />
                  </TableCell>
