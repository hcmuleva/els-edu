 ...
 * @param {any} value - Selected value (numeric ID)
...
export const CustomAsyncSelect = ({
    ...
    const fetchOptions = async () => {
      ...
      try {
        ...
        if (initialData && initialData.id) {
          const existsInData = data.some((opt) => opt.id === initialData.id);
          if (!existsInData) {
            mergedOptions = [initialData, ...data];
          }
        }
  ...
  const selectedOption =
    ...
    (initialData && initialData.id == value ? initialData : null);
