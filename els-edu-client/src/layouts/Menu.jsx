...
const PortalDropdown = ({ trigger, children, align = 'right', width = 'w-56' }) => {
    ...
    return (
        <>
            <div ref={triggerRef} onClick={handleToggle} className="cursor-pointer h-full w-full">
                {trigger}
            </div>
            {isOpen && createPortal(
                <div className="fixed inset-0 z-[9999] isolate" style={{ zIndex: 9999 }}>
                    ...
                    <div 
                        ...
                        style={{
                            ...
                            bottom: coords.bottom === 'auto' ? 'auto' : `${coords.bottom - 10}px`,
                            left: `${coords.left}px`,
                        }}
                    >
...
// Dropdown Item
const DropdownItem = ({ icon: Icon, label, onClick, className, variant = 'default' }) => (
    <div 
        onClick={(e) => {
            e.stopPropagation();
            if (onClick) onClick();
...
const CustomMenuItem = ({ to, primaryText, leftIcon, isOpen, className, ...props }) => {
    ...
    return (
        <Menu.Item
            ...
            className={cn(
                ...
                !isOpen && "mx-auto w-10 h-10 my-1 rounded-xl p-0 flex justify-center items-center",
                className
            )}
            sx={{
        ...
        return (
            <div className="mx-auto w-10 h-10 my-1 flex justify-center">
                <PortalDropdown
                    width="w-48"
                    trigger={
                        <div className="w-full h-full flex items-center justify-center rounded-xl hover:bg-primary/5 text-muted-foreground hover:text-primary transition-colors cursor-pointer" title={label}>
                            <Icon size={20} />
                        </div>
                    }
                >
...
const SectionHeader = ({ title, isOpen }) => {
    if (!isOpen) {
        return <div className="my-1 border-t border-border/50 w-6 mx-auto opacity-50" />;
    }
    return (
                ...
                <div className={cn(
                    ...
                    <button 
                        ...
                        className={cn(
                            ...
                            open ? "px-3 py-2 w-full min-h-[40px]" : "w-10 h-10 justify-center p-0"
                        )}
                            ...
                            <PortalDropdown
                                trigger={
                                    <div className={cn(
                                        ...
                                        {open && <ChevronDown size={14} className="text-muted-foreground group-hover:text-primary transition-colors shrink-0" />}
                                    </div>
                                }
                            >
                                 <div className="px-3 py-3 border-b border-border/50 mb-1 bg-muted/20">
                                    <p className="text-sm font-bold text-foreground truncate">{identity?.fullName}</p>
                                    <p className="text-xs text-muted-foreground opacity-80 truncate">{identity?.email}</p>
                                </div>
                                
                                <div className="py-1">
                                    <DropdownItem icon={User} label="Profile" onClick={() => notify('Profile clicked')} />
                                    <DropdownItem icon={Settings} label="Settings" onClick={() => notify('Settings clicked')} />
                                </div>
    
                                {uniqueRoles.length > 1 && (
                                    <div className="border-t border-border/50 my-1 pt-1">
                                        <p className="px-3 py-1.5 text-[10px] font-bold text-muted-foreground uppercase tracking-wider">Switch Role</p>
                                        {uniqueRoles.map(role => (
                                            <DropdownItem 
                                                key={role}
                                                icon={role === permissions ? ChevronRight : null}
                                                label={role}
                                                className={role === permissions ? "bg-primary/5 text-primary" : ""}
                                ...
                                <DropdownItem icon={LogOut} label="Logout" variant="destructive" onClick={handleLogout} />
                            </PortalDropdown>
                        </div>
                    </div>
                </div>
