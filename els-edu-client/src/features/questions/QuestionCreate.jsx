  ...
  const handleSave = async () => {
    try {
      ...
      const savePromises = questions.map((question) => {
        const { id, subject, topic, ...questionData } = question; // Remove local ID and single select fields
        // Add creator field
        questionData.creator = identity?.id;
        // Add subjects and topics arrays if they exist
        if (question.subjects && question.subjects.length > 0) {
          questionData.subjects = question.subjects;
        }
        if (question.topics && question.topics.length > 0) {
          questionData.topics = question.topics;
        }
        ...
        <div className="max-w-6xl mx-auto flex items-center justify-between">
          ...
          <div className="flex gap-3">
            ...
            >
              {isLoading
                ? "Saving..."
                : `Save ${questions.length > 1 ? "All" : "Question"}`}
            </button>
          </div>
        </div>
      </div>
