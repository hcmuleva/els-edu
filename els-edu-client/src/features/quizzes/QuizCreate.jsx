...
import { useCreate, useUpdate, useRedirect, useNotify, useGetIdentity, Title, useGetOne } from 'react-admin';
import { useParams } from 'react-router-dom';
import { 
    Plus, FileQuestion, ArrowLeft, Clock, Target, Shuffle, Eye, Sparkles, 
    BookOpen, Layers, CheckCircle, ChevronRight, ChevronLeft, Settings, Hash
} from 'lucide-react';
            ...
            return (
                <div key={index} className="flex items-center">
                    <div className="flex flex-col items-center gap-2">
                        ...
                        <span className={`text-xs font-bold uppercase tracking-wider ${isCurrent ? 'text-primary' : isCompleted ? 'text-green-600' : 'text-gray-400'}`}>
                            {step.label}
                        </span>
                    </div>
                    {index < steps.length - 1 && (
                        <div className={`w-24 h-0.5 mx-4 transition-all duration-500 ${isCompleted ? 'bg-green-500' : 'bg-gray-200'}`} />
                    )}
                </div>
            );
        })}
...
export const QuizCreate = () => {
    ...
    React.useEffect(() => {
        if (quizDataLoaded && isEdit) {
            setFormData({
                ...
                showCorrectAnswers: quizDataLoaded.showCorrectAnswers || 'after-submission',
    ...
    const handleNext = () => {
        ...
        if (currentStep === 1) {
            ...
            if (formData.passingScore === undefined || formData.passingScore < 0 || formData.passingScore > 100) {
                notify('Please enter a valid passing score (0-100)', { type: 'warning' });
                return;
            }
        }
    ...
    const handlePublish = async () => {
        ...
        try {
            // 1. Separate new and existing questions
            const newQuestions = questions.filter(q => q.isNew);
            const existingQuestionIds = questions
                .filter(q => !q.isNew)
                .map(q => q.existingId); // Use existingId for library questions

            // 2. Create new questions first
            const createdQuestionIds = [];
            
            // We use a loop to create questions sequentially to ensure we get IDs
            for (const question of newQuestions) {
                const { id, isNew, existingId, ...questionData } = question;
                
                // Prepare question payload
                const payload = {
                    ...questionData,
                    creator: identity?.id,
                    topic: formData.topic, // Link question to same topic as quiz
                    subject: formData.subject, // Link question to same subject as quiz
                    // Ensure options are valid for the question type                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       
                ...
                // Create the question
                // specific call to create, assuming useCreate's 'create' function returns the result or promise
                // In React Admin, mutateAsync or the promise returns { data: ... }
                const { data } = await create('questions', { data: payload }, { returnPromise: true });
                if (data && (data.documentId || data.id)) {
                    createdQuestionIds.push(data.documentId || data.id);
                }
            }

            // 3. Combine all question IDs
            const allQuestionIds = [...existingQuestionIds, ...createdQuestionIds];
            ...
            <div className="bg-white border-b border-border/50 px-6 py-4 sticky top-0 z-40 backdrop-blur-md bg-white/95 shadow-sm">
                <div className="max-w-5xl mx-auto flex items-center justify-between">
                    <div className="flex items-center gap-4">
                        <button 
                            onClick={currentStep === 0 ? () => redirect('/my-contents') : handleBack}
                        ...
                        >
                            {currentStep === 0 ? <ArrowLeft className="w-5 h-5" /> : <ChevronLeft className="w-5 h-5" />}
                        </button>
                        <div>
                    ...
                    {currentStep === 1 && (
                        <div className="p-10 space-y-10 animate-in slide-in-from-right-4 fade-in duration-300">
                            <div className="grid grid-cols-2 gap-8">
                                <div className="space-y-2">
                                    <label className="text-sm font-bold text-foreground">Quiz Type</label>
                                    <CustomSelect
                            ...
                            <div className="grid grid-cols-3 gap-6">
                                <div className="space-y-2">
                                    <label className="text-sm font-bold text-foreground flex items-center gap-2">
                                        <Clock className="w-4 h-4 text-blue-500" /> Time (min) <span className="text-red-500">*</span>
                                    </label>
                                    <input
                                        type="number"
                                        value={formData.timeLimit}
                                        onChange={(e) => updateFormData('timeLimit', parseInt(e.target.value) || 0)}
                                        className="w-full px-4 py-2.5 rounded-xl border border-border/50 focus:border-primary outline-none text-center font-bold"
                                    />
                                </div>
                                <div className="space-y-2">
                                    <label className="text-sm font-bold text-foreground flex items-center gap-2">
                                        <Target className="w-4 h-4 text-green-500" /> Pass Score % <span className="text-red-500">*</span>
                                    </label>
                                    <input
                                        type="number"
                            ...
                            <div className="bg-gray-50 p-6 rounded-2xl grid grid-cols-2 gap-4">
                                {[
                                    { key: 'isActive', label: 'Active', desc: 'Visible to students' },
                                    { key: 'isRandomized', label: 'Randomize', desc: 'Shuffle questions' },
                                    { key: 'allowReview', label: 'Allow Review', desc: 'Review answers later' },
                                ].map((item) => (
                                    <div 
                                        key={item.key} 
                                    ...
                                    >
                                        ...
                                        <div className={`w-11 h-6 rounded-full transition-colors relative ${formData[item.key] ? 'bg-primary' : 'bg-gray-200'}`}>
                                            <div className={`absolute top-1 w-4 h-4 rounded-full bg-white shadow-sm transition-all duration-300 ${formData[item.key] ? 'left-6' : 'left-1'}`} />
                                        </div>
                                    </div>
                                ))}
                                <div className="flex flex-col gap-1 p-3 bg-white rounded-xl border border-border/50">
                                    ...
                                    <CustomSelect
                                        ...
                                        onChange={(val) => updateFormData('showCorrectAnswers', val)}
                                        options={[
                        ...
                        <div className="animate-in slide-in-from-right-4 fade-in duration-300 min-h-[500px] flex flex-col">
                            <div className="p-4 border-b border-border/50 bg-gray-50 flex items-center justify-between sticky top-0 z-10">
                                <h3 className="font-bold text-foreground">
                                    Questions ({questions.length}) <span className="text-red-500">*</span>
                                </h3>
                                ...
                                {questions.length === 0 ? (
                                    <div className="flex flex-col items-center justify-center h-64 text-center">
                                        ...
                                        <p className="text-sm text-muted-foreground mt-1 max-w-xs">
                                            Start adding questions to your quiz using the buttons above.
                                        </p>
                                    </div>
                                ) : (
                                    <div className="space-y-4">
