...
export const SubjectEdit = () => {
  ...
  const { data: subject, isPending } = useGetOne(
    "subjects",
    { id },
    {
      meta: { populate: ["coverpage", "contents", "courses"] },
    }
  );
  ...
  useEffect(() => {
    if (subject) {
      setFormData({
        ...
        contents: subject.contents?.map((c) => c.id) || [],
      });
      ...
      if (subject.contents) {
        setSelectedContents(subject.contents);
      }
    }
  }, [subject]);
  ...
  const handleSelectContents = (contents) => {
  ...
  const removeContent = (contentId) => {
    ...
    setFormData((prev) => ({
      ...prev,
      contents: prev.contents.filter((id) => id !== contentId),
    }));
  };
  ...
  const handleSave = async () => {
    try {
      ...
      if (formData.contents.length > 0) {
        subjectData.contents = formData.contents;
      }
                ...
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-sm text-muted-foreground">
                      Select a course to associate with this subject
                    </p>
                  </div>
                </div>
                <CustomAsyncSelect
              ...
              <div className="space-y-4">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-sm text-muted-foreground">
                      Add learning contents to this subject
                    </p>
                  </div>
                  <button
                ...
                {selectedContents.length > 0 ? (
                  ...
                  <div className="flex flex-col items-center justify-center py-12 text-center border-2 border-dashed border-border/50 rounded-xl">
                    ...
                    <p className="text-sm text-muted-foreground">
                      Click "Browse Library" to add contents
                    </p>
                  </div>
                )}
              </div>
                ...
                <div className="space-y-6">
                  <div className="p-6 bg-gray-50 rounded-2xl border border-border/50 space-y-4">
                    <h3 className="font-bold text-foreground flex items-center gap-2">
                      <BookOpen className="w-5 is w-5 text-primary" />
                      Basic Information
                    </h3>
                  ...
                  <div className="p-6 bg-gray-50 rounded-2xl border border-border/50 space-y-4">
                    ...
                    <div className="space-y-4">
                      ...
                      <div>
                        ...
                        <p className="font-bold text-foreground">
                          {selectedContents.length} contents selected
                        </p>
                        {selectedContents.length > 0 && (
                          <div className="mt-2 space-y-1">
                            {selectedContents.slice(0, 3).map((c) => (
                              <div
                                key={c.id}
                            ...
                            {selectedContents.length > 3 && (
                              <p className="text-sm text-muted-foreground">
                                +{selectedContents.length - 3} more
                              </p>
                            )}
                          </div>
                        )}
