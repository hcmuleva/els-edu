"""
Initialization endpoint for setting up database collections
"""
...
async def initialize_quiz_questions(db=Depends(get_database)):
    """Initialize quiz questions in MongoDB"""
    from ..utils.database_utils import QUIZ_QUESTIONS
        ...
        try:
            for skill_type, questions in skill_types.items():
                for question_data in questions:
                    ...
                    inserted_count += 1
                    logger.info(f"Inserted quiz question for {category} - {skill_type}")
                    
        except Exception as e:
            error_msg = f"Failed to insert questions for '{category}': {str(e)}"
            errors.append(error_msg)
            logger.error(error_msg)
    
    return {
        "message": "Quiz questions initialization completed",
...
async def initialize_self_assessments(db=Depends(get_database)):
    """Initialize self-assessment questions in MongoDB"""
    ...
    for category, skill_types in ENHANCED_SELF_ASSESSMENT_QUESTIONS.items():
        try:
            ...
            inserted_count += 1
            logger.info(f"Inserted self-assessment for category: {category}")
            
        except Exception as e:
            error_msg = f"Failed to insert '{category}': {str(e)}"
            errors.append(error_msg)
            logger.error(error_msg)
    
    return {
        "message": "Self-assessment initialization completed",
        "inserted": inserted_count,
        "skipped": skipped_count,
        "errors": errors if errors else None
    }
